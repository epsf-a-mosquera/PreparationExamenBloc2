# ExamenBloc2/src/train_regression.py
"""
...
6. Standardisation des variables numériques (StandardScaler)
7. Sauvegarde les features finales dans REGRESSION_FEATURES_TRAIN_CSV_PATH et REGRESSION_FEATURES_TEST_CSV_PATH
...
import numpy as np
import os
from sklearn.preprocessing import OneHotEncoder, StandardScaler # StandardScaler pour la standardisation des données numériques et one-hot encoding des variables catégorielles
...
from sklearn.model_selection import train_test_split  # <-- ajouté (pour split avant preprocessing)
from src.config import CLEAN_CSV_PATH, REGRESSION_FEATURES_TRAIN_CSV_PATH, REGRESSION_FEATURES_TEST_CSV_PATH  # chemins définis dans config.py
...
# On prépare deux DF séparés pour fitter sur train et transformer test
...
# numerical colons (variables numériques continues uniquement, on exclut les bool)
...
# pour les variables booléennes, on peut les convertir en int (0/1)
...
if len(categorical_cols) > 0:
    ...
    # Pas de catégorielles -> pas d'encodage
    feature_names = []
    encoder = None

  
# --- Étape 6 : Standardisation des variables numériques (StandardScaler) ---
# Important : on NE standardise PAS les variables booléennes (converties en 0/1) pour conserver 0/1.
...
# pour vérification finale sanitaire
...
# Reconstruit un df complet (features + target) en gardant l'ordre d'index
df_train_full = pd.concat([df_train, y_train.rename("order_basket_value")], axis=1)
df_test_full  = pd.concat([df_test,  y_test.rename("order_basket_value")], axis=1)

df = pd.concat([df_train_full, df_test_full], axis=0).sort_index()
...
print(f"Features prêtes pour le ML sauvegardées dans {REGRESSION_FEATURES_TRAIN_CSV_PATH} et {REGRESSION_FEATURES_TEST_CSV_PATH}")

# IMPORTANT : on réutilise les bons X_train/X_test pour l'entraînement
X_train = df_train
X_test  = df_test


"""
Etape 2 - Entraînement du modèle ML de regression sur les features préparées.

Ce script :
1. Charge les features préparées par prepare_features.py (à partir du même df)
2. Sépare X et y (variable cible : order_basket_value), Sépare train/test
3. Entraîne un modèle LinearRegression
4. Évalue le modèle sur le test
5. Sauvegarde le modèle entraîné dans le dossier artifacts
6. Sauvegarde les métriques d'évaluation au format JSON dans artifacts
"""

import joblib
from src.config import ARTIFACTS_DIR
from sklearn.linear_model import LinearRegression
from src.config import REGRESSION_MODEL_PATH, REGRESSION_MODEL_META_PATH
...
# df = pd.read_csv(REGRESSION_FEATURES_TRAIN_CSV_PATH) --> pas necessaire de reimporter pandas car df_train_full est déjà défini plus haut
print("Features chargées :", df_train_full.shape)

# --- Étape 2 : Séparation X / y ---
# X_train, X_test, y_train, y_test sont déjà définis plus haut
# (split AVANT preprocessing pour éviter le data leakage)


# --- Étape 3 : Entraînement du modèle LinearRegression ---
model = LinearRegression()
model.fit(X_train, y_train)

# --- Étape 4 : Prédiction et score ---
y_pred = model.predict(X_test)
score = model.score(X_test, y_test)
print(f"Score du modèle sur le jeu de test : {score:.4f}")

# --- Étape 5 : Sauvegarde du modèle entraîné ---
...
# --- Étape 6 : Sauvegarde des métriques & métadonnées au format JSON pour un modèle de régression ---
metrics_path = REGRESSION_MODEL_META_PATH
os.makedirs(os.path.dirname(metrics_path), exist_ok=True)

# Prédictions régression (valeurs continues)
y_pred = model.predict(X_test)

# Métriques régression (JSON-friendly)
mae = float(mean_absolute_error(y_test, y_pred))
mse = float(mean_squared_error(y_test, y_pred))
rmse = float(np.sqrt(mse))
r2 = float(r2_score(y_test, y_pred))

# MAPE : peut exploser si y proche de 0 (ici basket_value min ~10 donc ok)
mape = float(mean_absolute_percentage_error(y_test, y_pred))

# Résidus (utile pour diagnostic)
...
# Interprétabilité : si modèle linéaire (Linear/Ridge/Lasso/ElasticNet)
...
meta = {
    ...
    "data": {
        ...
        "target_name": "order_basket_value",
        "split": {
            "test_size": 0.2,
            "random_state": 42,
