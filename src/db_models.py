...
from sqlalchemy.ext.declarative import declarative_base # Importer la base déclarative pour les modèles
...
class Event(Base):
    ...
    """
    __tablename__ = "events"  # Nom de la table dans la base de données

    event_id = Column(String(36), primary_key=True)  # ID unique de l'événement (UUID)
    event_time = Column(DateTime, nullable=False)  # Horodatage de l'événement
    ...
    order_id = Column(String(36), ForeignKey("orders.order_id"), nullable=False)  # ID de la commande associée à l'événement
    customer_customer_id = Column(String(36), ForeignKey("customers.customer_customer_id"), nullable=False) # ID du client associé à l'événement
    # Relations
...
class Order(Base):
    ...
    """
    __tablename__ = "orders"  # Nom de la table dans la base de données

    order_id = Column(String(36), primary_key=True)  # ID unique de la commande (UUID)
    customer_customer_id = Column(String(36), ForeignKey("customers.customer_customer_id"), nullable=False)  # ID du client associé à la commande
...
class Customer(Base):
    ...
    """
    __tablename__ = "customers"  # Nom de la table dans la base de données
